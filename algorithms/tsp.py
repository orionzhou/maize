...
from maize.formats.base import FileShredder, must_open
from maize.utils.iter import pairwise
from maize.apps.base import mkdir, which, sh
...
def plot_data(x, y, tour, M):
    from maize.graphics.base import plt, savefig
    plt.plot(x, y, "ro")
    for ia, ib in pairwise(tour):
...
def compare_lpsolve_to_concorde(POINTS=80, directed=False):
    from maize.algorithms.lpsolve import hamiltonian as lhamiltonian
