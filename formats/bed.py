...
def tiling(args):
    ...
    sp1.add_argument("--overlap", default=3000, type="int",
...
def chain(args):
    ...
    sp1.add_argument("--dist", default=100000, help="Chaining distance")
    p.set_outfile()
    opts, args = p.parse_args(args)
    ...
    for accn, bb in groupby(bed, key=lambda x: x.accn):
        ...
        for a, b in pairwise(bb):
            if a.seqid == b.seqid and b.start - a.end < args.dist:
                g.join(a, b)
        data = []
        for p in g:
            seqid = p[0].seqid
    ...
    newbed.print_to_file(args.outfile, sorted=True)
...
def alignextend(args):
    ...
    sp1.add_argument("--len", default=100, type="int",
                 help="Extend to this length")
    sp1.add_argument("--qv", default=31, type="int",
                 help="Dummy qv score for extended bases")
    sp1.add_argument("--bedonly", default=False, action="store_true",
    ...
    bedpe, ref = args
    qvchar = chr(args.qv + 33)
    pf = bedpe.split(".")[0]

    filtered = bedpe + ".filtered"
    if need_update(bedpe, filtered):
        filter_bedpe(bedpe, filtered, ref, rc=args.rc,
                     minlen=args.minlen, maxlen=args.maxlen, rlen=args.rlen)
...
def seqids(args):
    ...
    sp1.add_argument("--maxn", default=100, type="int",
                 help="Maximum number of seqids")
    sp1.add_argument("--prefix", help="Seqids must start with")
    sp1.add_argument("--exclude", default="random", help="Seqids should not contain")
    opts, args = p.parse_args(args)
...
def juncs(args):
    ...
    if len(args) > 1:
        ...
        infile = grouptbed.fn
        sh(cmd, infile=infile, outfile=args.outfile)
    else:
        sort([trimbed, "-o", args.outfile])
...
def mergebydepth(args):
    ...
    sp1.add_argument("--mindepth", default=3, type="int",
...
def longest(args):
    ...
    sp1.add_argument("--maxsize", default=20000, type="int",
                 help="Limit max size")
    sp1.add_argument("--minsize", default=60, type="int",
                 help="Limit min size")
    sp1.add_argument("--precedence", default="Medtr",
...
def fix(args):
    ...
    sp1.add_argument("--minspan", default=0, type="int",
...
def some(args):
    ...
    sp1.add_argument("-v", dest="inverse", default=False, action="store_true",
...
def uniq(args):
    ...
    sp1.add_argument("--sizes", help="Use sequence length as score")
    sp1.add_argument("--mode", default="span", choices=("span", "score"),
                 help="Pile mode")
    opts, args = p.parse_args(args)
    ...
    if args.sizes:
        sizes = Sizes(args.sizes).mapping
...
def bins(args):
    ...
    sp1.add_argument("--subtract",
                 help="Subtract bases from window [default: %default]")
    sp1.add_argument("--mode", default="span", choices=("span", "count", "score"),
                 help="Accumulate feature based on [default: %default]")
    sp1.add_argument("--nomerge", default=False, action="store_true",
...
def pile(args):
    ...
    sp1.add_argument("--minOverlap", default=0, type="int",
    ...
    iw = intersectBed_wao(abedfile, bbedfile, minOverlap=args.minOverlap)
    groups = Grouper()
    for a, b in iw:
...
def index(args):
    ...
    sp1.add_argument("--fasta", help="Generate bedgraph and index")
    sp1.add_argument("--query", help="Chromosome location")
    p.set_outfile()
...
def distance(args):
    ...
    sp1.add_argument("--distmode", default="ss", choices=("ss", "ee"),
    ...
    for a, b in pairwise(fp):
        ...
        dist, oo = range_distance(ar, br, distmode=args.distmode)
        total += 1
        if dist > 0:
            print(dist)
            valid += 1
...
def sample(args):
    ...
    sp1.add_argument("--raindrop", default=0, type="int",
                 help="Raindrop selection, ignores all other options")
    sp1.add_argument("--max", default=10, type="int",
                 help="Max depth allowed")
    sp1.add_argument("--targetsize", type="int",
    ...
    if raindrop:
        ...
        selected.print_to_file(args.outfile, sorted=True)
        return
...
def bedpe(args):
    ...
    sp1.add_argument("--span", default=False, action="store_true",
                 help="Write span bed file [default: %default]")
    sp1.add_argument("--strand", default=False, action="store_true",
            help="Write the strand columns [default: %default]")
    sp1.add_argument("--mates", help="Check the library stats from .mates file")
    opts, args = p.parse_args(args)
    ...
    pf = bedfile.rsplit(".", 1)[0]
    bedpefile = pf + ".bedpe"
    bedspanfile = pf + ".spans.bed" if args.span else None
    bed_to_bedpe(bedfile, bedpefile, \
                 pairsbedfile=bedspanfile, matesfile=args.mates, \
                 strand=args.strand)
...
def pairs(args):
    ...
    fp = open(bedfile)
    data = [BedLine(row) for i, row in enumerate(fp) if i < args.nrows]

    ascii = not args.pdf
    return bedfile, report_pairs(data, args.cutoff, args.mateorientation,
           pairsfile=args.pairsfile, insertsfile=insertsfile,
           rclip=args.rclip, ascii=ascii, bins=args.bins,
...
def summary(args):
    ...
    sp1.add_argument("--sizes", default=False, action="store_true",
...
def sort(args):
    ...
    sp1.add_argument("-i", "--inplace", dest="inplace",
            default=False, action="store_true",
            help="Sort bed file in place [default: %default]")
    sp1.add_argument("-u", dest="unique",
            default=False, action="store_true",
            help="Uniqify the bed file")
    sp1.add_argument("--accn", default=False, action="store_true",
    ...
    sortopt = "-k1,1 -k2,2n -k3,3n -k4,4" if not args.accn else \
              "-k4,4 -k1,1 -k2,2n -k3,3n"
    cmd = "sort"
    if args.tmpdir:
        cmd += " -T {0}".format(args.tmpdir)
    if args.unique:
        cmd += " -u"
...
def mates(args):
    ...
    sp1.add_argument("--lib", default=False, action="store_true",
            help="Output library information along with pairs [default: %default]")
    sp1.add_argument("--nointra", default=False, action="store_true",
            help="Remove mates that are intra-scaffold [default: %default]")
    sp1.add_argument("--prefix", default=False, action="store_true",
    ...
    pf = bedfile.rsplit(".", 1)[0]
    matesfile = pf + ".mates"
    lib = pf if args.lib else None
    fw = open(matesfile, "w")
    if lib:
        bedfile, stats = pairs([bedfile, \
                        "--rclip={0}".format(rclip),
                        "--cutoff={0}".format(args.cutoff)])
        sv = int(2 * stats.sd)
    ...
    for pe, lines in groupby(bed, key=key):
        ...
        if args.nointra and a.seqid == b.seqid:
            continue
...
def flanking(args):
    ...
    sp1.add_argument("--chrom", default=None, type="string",
            help="chrom name of the position in query. Make sure it matches bedfile.")
    sp1.add_argument("--coord", default=None, type="int",
            help="coordinate of the position in query.")
    sp1.add_argument("-n", default=10, type="int",
            help="number of flanking features to get [default: %default]")
    sp1.add_argument("--side", default="both", choices=("upstream", "downstream", "both"),
            help="which side to get flanking features [default: %default]")
    sp1.add_argument("--max_d", default=None, type="int",
            help="features <= max_d away from position [default: %default]")
    p.set_outfile()

    opts, args = p.parse_args(args)

    if any([len(args) != 1, args.chrom is None, args.coord is None]):
        sys.exit(not p.print_help())

    bedfile, = args
    position = (args.chrom, args.coord)
    n, side, maxd = args.n, args.side, args.max_d
...
if __name__ == '__main__':
    ...
    sp1 = sp.add_parser('depth', help='calculate average depth per feature using coverageBed',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
