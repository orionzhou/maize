...
from maize.formats.base import LineFile, BaseFile, must_open
from maize.formats.bed import Bed
from maize.formats.sizes import Sizes
from maize.utils.grouper import Grouper
from maize.utils.range import range_distance
from maize.apps.base import sh
from maize.formats.coords import print_stats
#from maize.utils.orderedcollections import OrderedDict
...
def filter(args):
    ...
    sp1.add_argument("--score", dest="score", default=0, type="int",
                 help="Score cutoff")
    p.set_align(pctid=95, hitlen=100, evalue=.01)
    sp1.add_argument("--noself", default=False, action="store_true",
                 help="Remove self-self hits")
    sp1.add_argument("--ids", help="Path to file with ids to retain")
    sp1.add_argument("--inverse", default=False, action="store_true",
...
def annotation(args):
    ...
    sp1.add_argument("--queryids", help="Query IDS file to switch [default: %default]")
    sp1.add_argument("--subjectids", help="Subject IDS file to switch [default: %default]")
    opts, args = p.parse_args(args)
...
def top10(args):
    ...
    sp1.add_argument("--top", default=10, type="int",
                help="Top N taxa to extract [default: %default]")
    sp1.add_argument("--ids", default=None,
...
def cscore(args):
    ...
    sp1.add_argument("--cutoff", default=.9999, type="float",
            help="Minimum C-score to report [default: %default]")
    sp1.add_argument("--pct", default=False, action="store_true",
            help="Also include pct as last column [default: %default]")
    sp1.add_argument("--writeblast", default=False, action="store_true",
...
def chain(args):
    ...
    sp1.add_argument("--dist", dest="dist",
...
def covfilter(args):
    ...
    sp1.add_argument("--scov", default=False, action="store_true",
            help="Subject coverage instead of query [default: %default]")
    sp1.add_argument("--supermap", action="store_true",
            help="Use supermap instead of union")
    sp1.add_argument("--ids", dest="ids", default=None,
            help="Print out the ids that satisfy [default: %default]")
    sp1.add_argument("--list", dest="list", default=False, action="store_true",
            help="List the id% and cov% per gene [default: %default]")
    sp1.add_argument("--iterby", dest="iterby", default="query", choices=allowed_iterby,
...
def best(args):
    ...
    sp1.add_argument("-n", default=1, type="int",
    ...
    sp1.add_argument("--hsps", default=False, action="store_true",
            help="get all HSPs for the best pair [default: %default]")
    sp1.add_argument("--subject", default=False, action="store_true",
...
if __name__ == '__main__':
    ...
    sp1 = sp.add_parser('completeness', help='print completeness statistics for each query',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = completeness)
    
    sp1 = sp.add_parser('annotation', help='create tabular file with the annotations',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = annotation)
    
    sp1 = sp.add_parser('top10', help='count the most frequent 10 hits',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = top10)
    
    sp1 = sp.add_parser('filter', help='filter BLAST file (based on score, id%%, alignlen)',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = filter)
    
    sp1 = sp.add_parser('covfilter', help='filter BLAST file (based on id%% and cov%%)',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = covfilter)
    
    sp1 = sp.add_parser('cscore', help='calculate C-score for BLAST pairs',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = cscore)
    
    sp1 = sp.add_parser('best', help='get best BLAST hit per query',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = best)
    
    sp1 = sp.add_parser('pairs', help='print paired-end reads of BLAST tabular file',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = pairs)
    
    sp1 = sp.add_parser('bed', help='get bed file from BLAST tabular file',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = bed)
    
    sp1 = sp.add_parser('condense', help='group HSPs together for same query-subject pair',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = condense)
    
    sp1 = sp.add_parser('chain', help='chain adjacent HSPs together',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = chain)
    
    sp1 = sp.add_parser('swap', help='swap query and subjects in BLAST tabular file',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = swap)
    
    sp1 = sp.add_parser('sort', help='sort lines so that query grouped together and scores desc',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = sort)
    
    sp1 = sp.add_parser('subset', help='extract hits from some query and subject chrs',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = subset)
    
    sp1 = sp.add_parser('mismatches', help='print out histogram of mismatches of HSPs',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = mismatches)
    
    sp1 = sp.add_parser('annotate', help='annotate overlap types in BLAST tabular file',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = annotate)
    
    sp1 = sp.add_parser('score', help='add up the scores for each query seq',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = score)
    
    sp1 = sp.add_parser('rbbh', help='find reciprocal-best blast hits',
            formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    sp1.add_argument('i', help = '')
    sp1.set_defaults(func = rbbh)
