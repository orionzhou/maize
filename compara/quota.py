...
from maize.utils.range import range_overlap
from maize.utils.grouper import Grouper
from maize.algorithms.lpsolve import GLPKSolver, SCIPSolver
from maize.compara.synteny import AnchorFile, _score, check_beds
...
if __name__ == '__main__':
    ...
    p.add_argument("qa_file", help = 'input qa file')
    p.add_argument("--quota", default="1:1",
    ...
    p.add_argument("--Nm", dest="Nmax", type=int, default=10,
            help="distance cutoff to tolerate two blocks that are "\
                    "slightly overlapping (cutoff for `quota mapping`) "\
                    "[units (gene or bp dist)]")
    p.add_argument("--self", dest="self_match", action="store_true",
    ...
    selected_ids = solve_lp(clusters, quota, work_dir=work_dir, \
            Nmax=args.Nmax, self_match=self_match, \
