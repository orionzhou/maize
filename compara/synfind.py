...
from maize.algorithms.lis import longest_increasing_subsequence, \
            longest_decreasing_subsequence
from maize.compara.synteny import check_beds, read_blast
...
def batch_query(qbed, sbed, all_data, args, fw=None, c=None, transpose=False):

    cutoff = int(args.cutoff * args.window)
    window = args.window / 2
    colinear = args.scoring == "collinear"
    qnote, snote = args.qnote, args.snote
...
if __name__ == '__main__':
    ...
    batch_query(qbed, sbed, all_data, args, fw=fw, c=c, transpose=False)
    if qbed.filename == sbed.filename:
        logging.debug("Self comparisons, mirror ignored")
    else:
        batch_query(qbed, sbed, all_data, args, fw=fw, c=c, transpose=True)
